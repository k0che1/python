...
'''
Продолжаем работать с датасетом StudentsPerformance.csv. Какое среднее значение оценок по всем предметам у мальчиков из такой же группы с уровнем образования,
как у родителей девочек, получивших максимальную среднюю оценку по всем предметам?
'''
df = pd.read_csv('StudentsPerformance.csv')
#группируемся по колонкам "пол абитуриента" и "уровень образования родителей"
#считаем среднее по предметам
a=df.groupby(['gender','parental level of education']).mean()
print(a,'\n')
b=a.loc['female']
print(b,'\n')
#для девочек ищем максимум среднего 
#получаем название группы "уровень образования родителей"
c=b.idxmax()
print(c[0],'\n')
#для мальчиков получаем среднюю оценку по этой группе.
e=a.loc[(  'male',c[0])].mean()
print(e)


'''
Для этого задания используется случайно генерируемая выборка из 100 записей об использовании велодорожек по датам. В выгрузке присутствуют данные только о 7 дорожках (в остальных пустые значения).
Посчитайте среднюю посещаемость каждой велодорожки отдельно по дням недели. Какой день недели (в среднем) наиболее посещаем?
'''
df = pd.read_csv('dataset_days-of-week.txt',sep=',')
#создаем колонку с днем недели
df['Date'] = pd.to_datetime(df['Date'])
df['day_of_week'] = df['Date'].dt.dayofweek
#сортируем посещяемость каждой велодорожки отдельно по дням недели, затем по всем дорожкам
